...
async def on_channel_member(ev: ChatMemberUpdated) -> None:
    ...
    ⚠️ Ловушка на будущее, замерена и оставлена намеренно: если `channel_id()`
    вернёт None (переменная не задана И `bot_settings` пуст, либо база на
    секунду недоступна — исключение там проглочено), условие ниже ложно, и
    событие из ЛЮБОГО канала считается нашим. Клубный обработчик его после
    этого уже не увидит. Раньше это било мимо (строку искали по админу, у
    которого её обычно нет), а теперь попадает в живого человека: у члена клуба
    строка 18+ есть почти всегда. Настоящее лекарство — такой же явный отвод
    клубного канала, как в `on_join_request`, но это правка вне рамки окна и
    отдельное решение: см. заявку в `ОТЧЁТ.md`.
    """
    known = channel_id()
    if known and str(ev.chat.id) != str(known):
