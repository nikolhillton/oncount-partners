...
def _set_source(sub: ChannelSubscriber, new_source: str) -> None:
    ...
    Новой семантики здесь нет: это дословный перенос правила, которое уже стояло
    в `on_join_request` (строка 313 базового кода), туда, где его не хватало, —
    в `ask_age`. Условие то же самое, и новый `source` на решение не влияет ни в
    одном из четырёх случаев ниже.
    ...
    * конкретная стоит, пришла другая конкретная (`dl:` против `jr:`, два
    ...
    Чего это правило НЕ умеет и что придётся читать глазами: человек, уже
    сидящий в канале (`status="in_channel"` с меткой `join_request`), кликнув
    ссылку рассылки, получает метку `dl:<код>` и приходит на карточку этой
    рассылки готовой единицей в «дошёл до канала» — хотя рассылка его туда не
    приводила. Строка одна на человека, столбца «когда поставили метку» нет, и
    отличить такого от настоящей конверсии в `tag_counts` нечем.

    Пустую метку не пишем вовсе. Сегодня её взяться неоткуда — все четыре вызова
    собирают значение как `f"dl:{tag}" if tag else "deeplink"` (и так же для
    `jr:`), потому что `clean_tag` на кириллице и пустом имени ссылки отдаёт "".
    Но пятый вызов, передавший сюда результат `clean_tag` напрямую, стёр бы
    известную метку пустотой, и заметить это было бы нечем.

    Прочитать `sub.source`, решить и записать — три шага, и «первое касание»
    держится на том, что между ними никто не вклинится. Держится это не само
    собой, а на устройстве бота: он живёт одной задачей в том же процессе, что и
    веб (`main.py` поднимает его через `asyncio.create_task`), а внутри блока
    `with SessionLocal()` у обоих вызывающих НЕТ ни одного `await` — значит,
    задача доходит до `commit`, не уступая управления соседней. Два события
    одного человека обрабатываются строго по очереди.

    Кто соберётся поставить сюда `await` (спросить что-то у Telegram между
    чтением и записью) или увести бота в отдельный процесс — обязан сначала
    перевести это условие в SQL (`UPDATE ... WHERE source IS NULL OR ...`),
    иначе метку рассылки затрёт тот, кто закоммитит вторым. Само условие
    стережёт `test_source_rule_relies_on_an_uninterrupted_block`.

    Второе окно шло этим путём и вернулось: Core-UPDATE защищал от гонки,
    которой в нынешнем устройстве нет, и приносил свою беду — забытый
    `WHERE telegram_id` переписывает метку ВСЕЙ таблице, и ни один прогон
    этого не замечает.
    """
    if not new_source:
        return
...
async def on_channel_member(ev: ChatMemberUpdated) -> None:
    ...
    Существующее поведение, которое стоит знать: если `channel_id()` вернёт None
    (переменная не задана И `bot_settings` пуст, либо база на секунду недоступна —
    исключение там проглочено), условие ниже ложно, и статус пишется по событию
    из ЛЮБОГО канала, включая клубный, а `SkipHandler` не поднимается. Отсюда
    практическое: стенд обязан ставить `NIKOL_CHANNEL_ID`, иначе ветка
    «чужой канал» просто не воспроизводится.
    """
    known = channel_id()
    if known and str(ev.chat.id) != str(known):
