...
# Только для УСПЕХА (200 у GET и у короткого HEAD): цифры живые, промежуточным
# кэшам их не отдаём. На отказ этот заголовок не ставится намеренно — 404 обязан
# быть неотличим от несуществующего адреса, а лишний заголовок и есть отличие.
...
# должен отвечать сам, и отвечать КОРОТКО — 200 без тела, без счёта меток и без
# строки в журнале (иначе Starlette выдаёт на него 405 с `Allow: GET`, то есть
# подтверждает анониму и адрес, и метод). Остальные пять обязаны отвечать 404,
# как и всё закрытое здесь, — и раньше проверки права, чтобы ответ не зависел
# от того, есть у спрашивающего ключ или нет.
...
def admin_api_channel_tags(request: Request,
    ...
    Отказ обязан быть НЕОТЛИЧИМ от несуществующего адреса — байт в байт: тот же
    404, то же тело `{"detail":"Not Found"}`, тот же набор заголовков, что у
    `GET /vydumannyj`. Поэтому у отказов заголовков не ставим ВООБЩЕ, включая
    `Cache-Control: no-store`: единственный лишний заголовок на 404 и есть
    подтверждение, что адрес существует, — а не подтверждать его и было целью.
    Приёмка 05.09 сняла прежний `no-store` на отказе именно за это.

    `no-store` остаётся там, где ему и место, — на успехе (200 у `GET` и у
    короткого `HEAD`): цифры живые, промежуточным кэшам их не отдаём. Отказ
    кэшировать нечего: в нём нет ни цифр, ни самого факта существования адреса.

    Методов ровно два. `HEAD` отвечает коротко: считать метки и писать в журнал
    «отдал N меток» он не идёт — им проверяют живость (мониторинг, само
    расписание ARDORIUM перед выгрузкой), и признак жизни, который пишется на
    каждый стук, перестаёт что-либо значить. В базу `HEAD` не ходит ВОВСЕ, когда
    вошёл по токену; по куке — ровно один SELECT партнёра внутри `require_admin`,
    как у любого `/admin/*`, и дешевле права не проверить. Обе цифры (0 и 1)
    закреплены в `test_head_with_right_is_short`.

    Отсюда прямо: 200 на `HEAD` значит «приложение поднято и ключ подходит», и
    НЕ значит «цифры считаются». Замерено: со сломанным `tag_counts` `HEAD`
    отвечает 200, а `GET` в тот же миг падает. Мониторинг, которому важно, что
    выгрузка жива, надо вешать на `GET`; `HEAD` — только признак жизни адреса.
    `OPTIONS` уходит в общий 404: CORS в приложении не настроен, из браузера
    этот адрес никто не читает, и предполётный запрос здесь означал бы только
    разведку. Остальное — 404 всем, включая владельца: писать сюда нечего, а
    `405 Allow: GET` рассказывал бы анониму то же, что и 200.

    Граница честно: на всех семи перечисленных методах отказ неотличим от
    несуществующего адреса, но слово ВНЕ списка (`TRACE`, `PROPFIND`, любое
    выдуманное) до тела не доходит — Starlette отвечает на него своим 405 с
    `Allow` раньше (`starlette/routing.py:288-296`), и вот ЭТИМ адрес себя
